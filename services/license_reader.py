# services/license_reader.py - Cleaned with Original Function Names
...
def _get_cached_result(image_path: str, method: str):
    try:
...
def _resize_image_optimal(image: np.ndarray) -> np.ndarray:
    ...
    if (640 <= w <= OPTIMAL_WIDTH and 480 <= h <= OPTIMAL_HEIGHT):
        return image
...
def cleanup_all_temp_files():
    ...
    for filepath in _temp_files[:]:
        safe_delete_temp_file(filepath)
...
def extract_name_from_lines(image_path: str, reference_name: str = "", best_ocr_match: str = "", match_score: float = 0.0) -> Dict[str, str]:
    """Extract name from lines with verification"""
...
def _detect_name_pattern(raw_text: str) -> Optional[str]:
    ...
    filter_keywords = [
        ...
        'CITY', 'PROVINCE', 'BARANGAY', 'STREET', 'ROAD', 'AVENUE', 'LN', 'FNMN',
        'RESIDENCIA', 'BLK', 'LOT' 
]
...
def package_name_info(structured_data: Dict[str, str], basic_text: str, fingerprint_info: Optional[dict] = None) -> NameInfo:
    """Package name info into NameInfo object"""
    return NameInfo(
...
def get_guest_name_from_license_image(image_path: str) -> str:
    """Get guest name from license image"""
    try:
...
def auto_capture_license_rpi(reference_name: str = "", fingerprint_info: Optional[dict] = None, retry_mode: bool = False) -> Optional[str]:
    """Auto-capture license using RPi Camera with ROI-only enhancement (no oscillation)"""
    camera = get_camera()
    ...
    STABILITY_FRAMES = 1  # Need 5 good readings to go green
    ...
    def _enhance_roi_only(roi, keyword_count):
        """Enhance only the ROI based on keyword detection - no global changes"""
        if keyword_count == 0:
    ...
    try:
        while True:
            ...
            enhancement_info = f" (ROI: {roi_enhancement_level:.1f}x)"
            
            if ready_to_capture:
                box_color = (0, 255, 0)
                remaining_delay = CAPTURE_DELAY - (time.time() - ready_time) if ready_time else CAPTURE_DELAY
                green_duration = f" [Green: {green_time:.1f}s]"
            ...
            # Optional: Show enhanced ROI preview in corner
...
def licenseRead(image_path: str, fingerprint_info: dict) -> NameInfo:
    """Read and verify license for students"""
...
def licenseReadGuest(image_path: str, guest_info: dict) -> NameInfo:
    """Read and verify license for guests"""
...
def complete_verification_flow(image_path: str, fingerprint_info: dict, 
    ...
    """Complete verification flow for students"""
...
def complete_guest_verification_flow(image_path: str, guest_info: dict,
    ...
    """Complete verification flow for guests"""
...
def _retake_prompt(expected_name: str, detected_name: str) -> bool:
    """Get user confirmation for retake"""

...
def get_frame_from_ip_webcam(url):
    """Get frame from IP webcam URL"""
    try:
...
def verify_helmet(webcam_url='http://192.168.100.235:8080/shot.jpg'):
    """Verify full-face helmet for specified duration with improved camera handling"""
    if session is None:
    ...
    print(f"📱 Looking for helmet for {HELMET_DETECTION_DURATION} seconds...")
    ...
    while True:
        ...
        frame = get_frame_from_ip_webcam(webcam_url)
        ...
        if frame is None:
            ...
            cv2.putText(error_frame, "Check IP webcam connection", (120, 280),
        ...
        if full_face_detected and not nutshell_detected:
            ...
            if detection_start is not None:
                ...
                remaining = max(0, HELMET_DETECTION_DURATION - elapsed)
